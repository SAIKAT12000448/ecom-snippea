...
const ElectronicsItem = () => {
    ...
    useEffect(() => {
        fetch('https://corp.quirkybuy.com/api/filterProducts?category=1')
            .then(res => res.json())
            .then(data => {
    ...
    return (
        <div className='container mx-auto my-5 mt-8'>
            <div className='flex justify-between mx-10'>
                <h1 className="text-left text-4xl antialiased font-bold my-4">{categories.length > 0 && categories[0].category_name}</h1>
                <h1><Link to='/categories/1'><p className='text-right text-xl px-5'>view more</p></Link></h1>
            </div>
